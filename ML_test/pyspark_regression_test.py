...
def prepare_data(df: DataFrame, assembler) -> DataFrame:
    df = df.withColumn("married", f.col("married").cast(IntegerType()) )
...
def vector_assembler() -> VectorAssembler:
    features = ['age', 'married', 'salary', 'successfully_credit_completed', 'credit_completed_amount',
...
def train_model(train_df, test_df) -> (RandomForestRegressionModel, float): # type: ignore
